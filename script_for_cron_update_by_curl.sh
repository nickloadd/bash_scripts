...
local_name=$(ls $dir/ | grep -E "GeoLite2-Country_.*.tar.gz")

#Body
if [ -z $local_name ] || [ "$local_name" != "$remote_name" ] ; then
  echo "Downloading maxmind db version \"${remote_name%.*.*}\""
  curl --connect-timeout 30 --retry-delay 5 --retry 5 -s --output $dir/$remote_name -J $url
  if [ $? != 0 ]; then echo "[ERR] download error" && exit 1; fi
  tar -xzf $dir/$remote_name --strip-components=1  -C $dir/ `tar -tzf $dir/$remote_name | grep -E ".*.mmdb"`
  if [ $? != 0 ]; then echo "[ERR] extract error" && exit 1; fi
  if [ -z $local_name ]; 
    then echo "Successful download maxmind db \"${remote_name%.*.*}\" version"
  else
    rm -rf $dir/$local_name && echo "Successful update maxmind db to \"${remote_name%.*.*}\" version"; 
  fi
else
